...
**`baseline` re-pins on a re-attempt, for a `worktree` job only (v3.6.3).** `register-lane` pinned this SHA once and, before 3.6.3, never rewrote it — only `resume-prepare` (called by `/v:resume`) cleared it. That left a door `/v:resume` does not guard: a human re-running `/v:dispatch` on the same run directory after a halt gets a new worktree branched from the current `HEAD`, but the gate still diffs against the *first* attempt's pin, so every bookkeeping file the pipeline committed between attempts (`manifest.yaml`, `dispatch.workflow.js`, `state.json`, `results/<id>.json`, `preexisting/<id>.txt`) is charged to the job as an out-of-lane write — the same failure [`commands/v-resume.md`](../../commands/v-resume.md) step 4 already names as finding 146 (2026-09-03), reached through the un-guarded door. `register-lane` now detects a concluded previous attempt for a `worktree` job — a receipt `receipts/<id>.gate.json` or a result `results/<id>.json` already exists and the job is not `merged.integrated` — and treats registration as a **new attempt**: it archives the old receipt as `receipts/<id>.gate.superseded-<tag>.json` (the convention `resume-prepare` already uses), drops the stale `baseline` here and the stale lane-map pin, discards the stale before-image so `provision_command` re-runs and re-photographs the fresh tree, and pins the new worktree's `HEAD`. The signal is pipeline-written (a receipt or result on disk), never the worker's self-report, so the one-shot rule that stops a worker from re-registering after committing stays intact. A `direct` job re-pins nothing automatically — a direct worker can write anywhere in the checkout, including a forged result, so its pin stays one-shot and the ack carries a stale-pin warning naming `resume-prepare`; `/v:resume` remains the documented path for it.
