...
**A paused Engine C run is not a `timeout`.** The native Workflow runtime *pauses* — never fails — an agent that is waiting out the operator's claude.ai usage limit (docs: code.claude.com/docs/en/workflows, "When a run hits your usage limit," fetched 2026-09-21), and a paused agent produces exactly the same no-git/no-filesystem-progress signal a real hang does — `compound-v-liveness.py` has no runtime-written pause marker to read instead (checked: no such file exists under a session's `~/.claude/projects/` run directory) and so cannot tell the two apart. It folds a pause into the same `STALE` → `timeout` path as a genuine hang, but on a run carrying `dispatch.workflow.js` or `lane-map.json` (an Engine C dispatch) the probe appends a `PAUSED?` hint to the reason text and an `attention_hint` field in `--json` — the operator's cue to check the `/workflows` header for a usage-limit reset time *before* treating the job as dead. This never applies in `claude -p`, a background session, Remote Control, or an agent-team teammate session: a run cannot pause in any of those, so the affected `agent()` call just resolves to `null`/fails instead, and it is the **existing** retry/escalation ladder that catches it — the emitted script's `withRetry` (`RETRY_MAX_ATTEMPTS_DEFAULT = 3` in `scripts/compound-v-emit-workflow.py`) and, for a review job that exhausts that budget, the one-shot `CLAUDE_ESCALATION = ("sonnet", "opus", "fable")` lift described above — never a second, pause-aware mechanism.
